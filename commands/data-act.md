...
1. Read `~/.claude/skills/data-act/SKILL.md` and follow its workflow exactly. The orchestrator handles verbatim citation, gotchas, house style, template filling, and Word rendering.
...
5. Default output is the chat itself. After the chat answer, ask the lawyer whether to export to Word. If yes, render via `python3 ~/.claude/skills/data-act/scripts/render_docx.py --template <md> --deliverable-type <type>`. The renderer:
